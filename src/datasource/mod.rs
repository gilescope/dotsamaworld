...
// pub async fn get_parachain_id<S: Source>(source: &mut S) -> Result<Option<NonZeroU32>,
// polkapipe::Error> { 	if is_relay_chain(source.url()) {
// 		return Ok(None);
// 	}
...
// pub async fn get_parachain_id_from_url<S: Source>(source: &mut S) -> Result<Option<NonZeroU32>,
// polkapipe::Error> { 	get_parachain_id(source).await
// }
	...
	R: Future<Output = ()> + 'static,
{
	pub async fn watch_blocks(
		...
		// sender: Option<HashMap<NonZeroU32, crossbeam_channel::Sender<(RelayBlockNumber, i64,
		// H256)>>>,
	...
	let mut ext: Vec<(Option<DataEntity>, Vec<DataEvent>)> = ext
		...
		.map(|extrinsic| {
			let eid = if let DataEntity::Extrinsic {
				details: Details { doturl: DotUrl { extrinsic: Some(eid), .. }, .. },
				..
			} = extrinsic
			{
				eid
...
mod tests {
	...
	//     let msg =
	// "02100104000100000700c817a8040a13000100000700c817a804010300286bee0d01000400010100353ea2050ff562d3f6e7683e8b53073f4f91ae684072f6c2f044b815fced30a4"
	// ;     let result =
	...
	fn decode_events_kusama_10_000_000() {
		...
		let _val = decoder::decode_value_by_id(&metad, &entry.ty, &mut bin.as_slice()).unwrap();
