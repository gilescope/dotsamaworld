...
async fn get_desub_metadata(url: &str, version: Option<(String, H256)>) -> Metadata {
    ...
    if let Some((version, hash)) = version {
        ...
        params = Some(jsonrpsee_types::ParamsSer::Array(vec![subxt::rpc::JsonValue::String(hex::encode(hash.as_bytes()))]));
    ...
    Metadata::from_bytes(&metadata_bytes).expect(&format!("should be able to get metadata from {}", &url))
}
    ...
    >: From<Client<T>>,
{
    ...
    if let Ok(contents) = std::fs::read(&filename) {
        ...
        if let Some(block_hash) = api
            ...
            .await
            .unwrap() {
                std::fs::write(&filename, &hex::encode(block_hash.as_bytes())).expect("Couldn't write event output");
                Some(block_hash)
        ...
        if para_id.is_some() {
            ...
            while let Ok((block_number, block_hash)) = recieve_channel.recv() {
                ...
                //     desub_current::decoder::decode_extrinsics(&metad, &mut bytes.as_slice())
                {
                    ...
                    let events = get_events_for_block(&api, &url, block_hash, block_number, &None).await?;
                    let current = handle
                        .0
            ...
            =
                ...
                while let Some(Ok(block_header)) = block_headers.next().await {
                    ...
                    if let Ok(Some(block_body)) = api.client.rpc().block(Some(block_hash)).await {
                        ...
                        println!("got block oh yeeet, we live yoloing!!! {} {} ", ext_clone.len() ,hex::encode(block_hash.as_bytes()));
...
async fn get_extrinsics(block_number: u32, url: &str, api: &RuntimeApi<DefaultConfig, DefaultExtraWithTxPayment<DefaultConfig, subxt::extrinsic::ChargeTransactionPayment<DefaultConfig>>>, block_hash: H256) -> Vec<Vec<u8>> {
    let urlhash = please_hash(&url);
    let path = format!("target/{urlhash}.metadata.scale.events");
    let _ = std::fs::create_dir(&path);
    ...
    // if let Ok(contents) = std::fs::read(&filename) {
    //     let exs = String::from_utf8(contents).unwrap().lines().map(|ex| hex::decode(ex).unwrap()).collect();
    //     Some(exs)
    // } else {
        ...
        if let Ok(Some(block_body)) = api.client.rpc().block(Some(block_hash)).await {
            ...
            let vals = block_body.block.extrinsics.iter().fold(String::new(), |mut buf, ex| {
...
async fn get_events_for_block(api: &polkadot::RuntimeApi<DefaultConfig, DefaultExtra<DefaultConfig>>,
    url: &str, blockhash: H256, blocknum: u32, sender: &Option<HashMap<NonZeroU32, crossbeam_channel::Sender<(RelayBlockNumber, H256)>>>)  -> Result<Vec<DataEvent>, Box<dyn std::error::Error>> {
    ...
    let metad =
    async_std::task::block_on(get_desub_metadata(&url, Some((version.unwrap(), blockhash))));
    ...
    let bytes = if let Ok(contents) = std::fs::read(&filename) {
        ...
        if let Some(sp_core::storage::StorageData(events_raw)) = call {
            //
            // TODO why can we not decode events here????????????????????????
            //
            ...
            // ) {
                ...
                std::fs::write(&filename, &hex::encode(&events_raw)).expect("Couldn't write event output");
            // } else {
            //     println!("Can't decode so won't cache. {}", blocknum);
            // }
            Some(events_raw)
        } else {
            None
    ...
    if let Some(events_raw) = bytes {
        ...
        let version = get_metadata_version(&api.client, &url, blockhash, blocknum).await.unwrap();
        let metad =
    ...
    //     let bin = events_raw.clone();//hex::decode(encoded_events).unwrap();
    //     println!("bin{}", bin.len());
    // let storage = decoder::decode_storage(&metad);
        ...
        //     .to_runtime_api::<polkadot::RuntimeApi<DefaultConfig, DefaultExtra<DefaultConfig>>>();
        {
            if let Ok(mut event_sub) = api.events().subscribe_finalized().await {
                ...
                while let Some(events) = event_sub.next().await {
                    ...
                    println!("got event oh yeeet, we yoloing!!! {}" ,hex::encode(blockhash.as_bytes()));
...
mod tests {
    ...
    fn decode_events() {
        let metad =
            async_std::task::block_on(get_desub_metadata("wss://statemint-rpc.polkadot.io:443", None));
    ...
    fn get_extrinsics_test() {
        ...
        if let Ok(extrinsic) = decoder::decode_unwrapped_extrinsic(&metad, &mut results[0].as_slice())
        {
            println!("{:#?}", extrinsic);
        } else { 
