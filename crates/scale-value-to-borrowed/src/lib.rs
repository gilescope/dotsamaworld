...
pub fn convert<T>(input: &'_ scale_value::Value<T>, optimise: bool) -> scale_borrow::Value<'_> {
    match &input.value {
        scale_value::ValueDef::Primitive(p) => match &p {
            scale_value::Primitive::U128(uint) => {
                if optimise {
                    if let Ok(uu64) = u64::try_from(*uint) {
                        scale_borrow::Value::U64(uu64)
                    } else {
                        scale_borrow::Value::U128(Box::new(*uint))
                    }
                } else {
                    scale_borrow::Value::U128(Box::new(*uint))
                }
            }
            scale_value::Primitive::Bool(uint) => scale_borrow::Value::Bool(*uint),
            scale_value::Primitive::Char(uint) => scale_borrow::Value::Char(*uint),
            scale_value::Primitive::String(uint) => scale_borrow::Value::Str(uint.as_str()),
            scale_value::Primitive::U256(uint) => scale_borrow::Value::U256(uint),
            scale_value::Primitive::I256(uint) => scale_borrow::Value::I256(uint),
            scale_value::Primitive::I128(uint) => scale_borrow::Value::I128(Box::new(*uint)),
        },
        scale_value::ValueDef::Composite(comp) => convert_composite(comp, optimise),
        scale_value::ValueDef::Variant(var) => scale_borrow::Value::Object(Box::new(vec![(
            var.name.as_str(),
...
fn convert_composite<T>(
    ...
    match input {
        scale_value::Composite::Named(comp) => {
            ...
            scale_borrow::Value::Object(Box::new(result))
        }
        scale_value::Composite::Unnamed(comp) => {
                ...
                })
            {
                ...
                let v: Vec<u8> = comp
                    ...
                    .collect();
                return scale_borrow::Value::ScaleOwned(Box::new(v));
            }
